...
struct XcfMcpActionHandler {
    ...
    static func handleAction(action: String) async -> String {
        ...
        case let cmd where cmd.starts(with: Actions.snippet + Format.spaceSeparator) || cmd == Actions.snippet:
            return handleSnippetAction(action: action)

        case let cmd where cmd.starts(with: Actions.readdir + Format.spaceSeparator) || cmd == Actions.readdir:
            return handleReadDirAction(action: action)

        case let cmd where cmd.starts(with: Actions.read + Format.spaceSeparator) || cmd == Actions.read:
            return handleReadAction(action: action)

        case let cmd where cmd.starts(with: Actions.cd + Format.spaceSeparator) || cmd == Actions.cd:
            return handleCdAction(action: action)
    ...
    // MARK: - File and Directory Actions

    /// Handles the snippet action to extract code snippets
    /// - Parameter action: The action string (e.g., "snippet main.swift 10 20")
    /// - Returns: The extracted code snippet or error message
    static func handleSnippetAction(action: String) -> String {
        let parts = action.components(separatedBy: Format.spaceSeparator)
        guard parts.count >= 2 else {
            return "Usage: snippet <filePath> [startLine endLine]"
        }
        ...
        guard FileManager.default.fileExists(atPath: resolvedPath) else {
            return String(format: ErrorMessages.errorReadingFile, "File not found: \(filePath)")
        }

        var result = warning.isEmpty ? "" : warning + Format.newLine + Format.newLine

        if parts.count >= 4, let startLine = Int(parts[2]), let endLine = Int(parts[3]) {
            // Line range snippet
            ...
            result += String(format: McpConfig.codeBlockFormat, language, snippet)
        } else {
            // Entire file
            do {
                ...
                result += String(format: McpConfig.codeBlockFormat, language, fileContents)
            } catch {
    ...
    /// Handles the read action to read file contents
    /// - Parameter action: The action string (e.g., "read main.swift")
    /// - Returns: The file contents or error message
    static func handleReadAction(action: String) -> String {
        let parts = action.components(separatedBy: Format.spaceSeparator)
        guard parts.count >= 2 else {
            return "Usage: read <filePath>"
        }

        let filePath = parts.dropFirst().joined(separator: Format.spaceSeparator)

        guard let projectDir = XcfXcodeProjectManager.shared.currentFolder else {
            return "No current project directory set"
        }
    ...
    /// Handles the readdir action to list directory contents
    /// - Parameter action: The action string (e.g., "readdir src swift")
    /// - Returns: The directory listing or error message
    static func handleReadDirAction(action: String) -> String {
        let parts = action.components(separatedBy: Format.spaceSeparator)

        guard let currentFolder = XcfXcodeProjectManager.shared.currentFolder else {
            return "No current folder is set. Please select a project first."
        }
        ...
        if parts.count >= 2 {
            ...
            fileExtension = parts.count >= 3 ? parts[2] : nil
        } else {
            directoryPath = currentFolder
            fileExtension = nil
        }
        ...
        do {
            ...
            return filePaths.joined(separator: McpConfig.newLineSeparator)
        } catch {
            return String(format: ErrorMessages.errorReadingDirectory, error.localizedDescription)
        }
    }

    /// Handles the cd action to change the current directory
    /// - Parameter action: The action string (e.g., "cd src")
    /// - Returns: A message indicating the result
    static func handleCdAction(action: String) -> String {
        let parts = action.components(separatedBy: Format.spaceSeparator)
        ...
        guard parts.count >= 2 else {
            return "Current directory: \(currentFolder)"
        }
        ...
        do {
            ...
            return String(format: ErrorMessages.errorChangingDirectory, error.localizedDescription)
        }
    }
